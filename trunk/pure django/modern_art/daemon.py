...
#from essayridge.account.models import UserInfo
"""
from essayridge.editor.schedule import cron_job
class MyCron(BaseCron):
    def __init__(self,pid):
        BaseCron.__init__(self,pid)
        self.add_event("test_job",15,"minute",round=True)
        #self.add_event("test_job",5,"second",round=True)
    def test_job(self):
        now=datetime.datetime.now()
...
from modern_art.user_logic.gamelogic import *
from modern_art.user_logic.models import Auction
class MyCron(BaseCron):
    def __init__(self,pid):
        BaseCron.__init__(self,pid)
        #self.add_event("deal_job",15,"minute",round=True);
        self.add_event("deal_job",1,"minute",round=True);
        #self.add_event("repurchase_job",30,"second",round=True);
    def repurchase_job(self):
        now=datetime.datetime.now()
        #user = User.objects.get(id='31')
        print 'RUNNING REPURCHASE JOB -------------------'
        RepurchaseButton()
        print now
        print 'END REPURCHASE JOB -------------------'
        print '\n'
    def deal_job(self):
        now=datetime.datetime.now()
        #user = User.objects.get(id='31')
        print 'RUNNING DEAL JOB -------------------'
        DealButton()
        print now
        print 'END DEAL JOB -------------------'
        print '\n'
...
start your
class here. inherit it from BaseCron above
