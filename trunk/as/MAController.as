		...
		private function gotResult(result_value : Object) : void {
			...
			switch(status_code) {
				...
				case MAConst.AUCTION_SUCCESS:
					...
					view.centerView.showPanel.card.button.play();
...
//                case MAConst.AUCTION_ERROR:
				...
				case MAConst.BID_SUCCESS:
					...
					if (card && card["content"] is AuctionModel && card["content"].card.auction_type != 3) {
						getAuction(card["content"].id, model.player.id);
					}
					break;
...
getPlayerInfo("crossin");
		...
		public function getAuction(id : int, player_id : int) : Boolean {
			nc.call("game.getAuction", responder, id, player_id);
			return true;
		}
